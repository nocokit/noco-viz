<template>
  <div class="backup-restore">
    ...
    <!-- Modal: Backup -->
    <div class="modal-overlay" :class="{ open: showBackupModal }">
      <div class="modal">
        <div class="modal-header">
          <h3>新建系统备份</h3>
          <div style="cursor:pointer; color:#888" @click="closeBackupModal">✕</div>
        </div>
    ...
    <div class="modal-overlay" :class="{ open: showRestoreModal }">
      <div class="modal" style="border-color:var(--danger)">
        <div class="modal-header">
          <h3 style="color:var(--danger)">⚠️ 确认恢复系统？</h3>
          <div style="cursor:pointer; color:#888" @click="closeRestoreModal">✕</div>
        </div>
        <div class="modal-body">
...
.btn {
    ...
    cursor: pointer; display: flex; align-items: center; gap: 6px; border: 1px solid var(--border);
    background: var(--bg-card); color: var(--text-secondary); transition: 0.2s;
}
...
.modal-overlay {
    ...
    background: rgba(0,0,0,0.7); backdrop-filter: blur(4px); z-index: 100;
    display: none; justify-content: center; align-items: center;
}
.modal-overlay.open { display: flex; }

.modal {
    background: var(--bg-card); width: 480px; border-radius: 12px; border: 1px solid var(--border); box-shadow: 0 20px 50px rgba(0,0,0,0.5);
    display: flex; flex-direction: column;
}
.modal-header { padding: 20px 24px; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; }
.modal-header h3 { margin: 0; font-size: 16px; color: #fff; font-weight: 600; }
.modal-body { padding: 24px; }
.modal-footer { padding: 16px 24px; border-top: 1px solid var(--border); display: flex; justify-content: flex-end; gap: 12px; background: var(--bg-sidebar); border-radius: 0 0 12px 12px; }
