...
// 使用 ref 包装模板数据，使其可以被修改
const officialTemplatesList = ref(officialTemplates)
const sharedTemplatesList = ref(sharedTemplates)

// 筛选后的官方模板
const filteredOfficialTemplates = computed(() => {
  if (activeFilter.value === 'all' || activeFilter.value === 'official') {
    return officialTemplatesList.value
  }
  return []
})
...
const filteredSharedTemplates = computed(() => {
  if (activeFilter.value === 'all' || activeFilter.value === 'shared') {
    return sharedTemplatesList.value
  }
  return []
})
...
const handlePublishSuccess = (template) => {
  // 将新模板添加到共享模板列表
  sharedTemplatesList.value.unshift(template)
  ...
  ElMessageBox.confirm(
    ...
    .then(() => {
      ...
      if (template.type === 'official') {
        const index = officialTemplatesList.value.findIndex(t => t.id === template.id)
        if (index !== -1) {
          officialTemplatesList.value.splice(index, 1)
          ElMessage.success('模板已删除')
        }
      } else {
        const index = sharedTemplatesList.value.findIndex(t => t.id === template.id)
        if (index !== -1) {
          sharedTemplatesList.value.splice(index, 1)
          ElMessage.success('模板已删除')
        }
      }
    })
