      ...
      <div class="ruler-container">
        ...
        >
          ...
          <CanvasToolbar
            ...
            :floating="true"
            @toggle-grid="gridConfig.enabled = !gridConfig.enabled"
            @change-grid-type="gridConfig.type = $event"
            @auto-layout="handleAutoLayout"
            @zoom-out="zoomOut"
            @zoom-in="zoomIn"
            @zoom-set="setZoom"
...
import CanvasToolbar from './editor/components/CanvasToolbar.vue'
...
import { gridTypeOptionsConfig } from './editor/config/toolbarConfig'
...
const handleSave = async () => {
  ...
  const canvasConfig = {
    ...
    backgroundPosition: pageConfig.backgroundPosition,
    backgroundWidth: pageConfig.backgroundWidth,
    backgroundHeight: pageConfig.backgroundHeight,
...
const pageConfig = reactive({
  ...
  backgroundSizeMode: 'auto',  // 新增：尺寸模式
  backgroundWidth: '',
  backgroundHeight: '',
  title: '数据大屏'
})
...
const updatePageConfig = (value) => {
  // 如果设置了背景图片，且吸附方向和背景尺寸为空或默认值，则初始化
  if (value.backgroundImage && value.backgroundImage !== pageConfig.backgroundImage) {
    // 如果吸附方向为空或默认值，设置为"居中上"
    if (!pageConfig.backgroundPosition || pageConfig.backgroundPosition === 'center center') {
      value.backgroundPosition = 'center top'
    }
    // 如果背景尺寸模式为空，设置为 auto
    if (!pageConfig.backgroundSizeMode) {
      value.backgroundSizeMode = 'auto'
      value.backgroundWidth = 'auto'
      value.backgroundHeight = 'auto'
    }
  }

  // 根据尺寸模式自动设置宽高
  if (value.backgroundSizeMode) {
    switch (value.backgroundSizeMode) {
      case 'cover':
      ...
      case 'auto':
        ...
        break
      case 'half':
        value.backgroundWidth = '50%'
        value.backgroundHeight = '50%'
        break
      case 'custom':
        // 自定义模式，保持用户输入的值
        break
...
const screenContainerStyle = computed(() => {
  ...
  if (pageConfig.backgroundImage) {
    ...
    style.backgroundPosition = pageConfig.backgroundPosition || 'center center'

    // 根据尺寸模式处理背景尺寸
    const mode = pageConfig.backgroundSizeMode || 'auto'
    switch (mode) {
      case 'cover':
      ...
      case 'custom':
        // 自定义模式，使用用户输入的宽高
        if (pageConfig.backgroundWidth || pageConfig.backgroundHeight) {
          const width = pageConfig.backgroundWidth || 'auto'
          const height = pageConfig.backgroundHeight || 'auto'
...
const restoreProjectData = (projectData) => {
  ...
  if (projectData.backgroundPosition) pageConfig.backgroundPosition = projectData.backgroundPosition
  if (projectData.backgroundWidth) pageConfig.backgroundWidth = projectData.backgroundWidth
  if (projectData.backgroundHeight) pageConfig.backgroundHeight = projectData.backgroundHeight
