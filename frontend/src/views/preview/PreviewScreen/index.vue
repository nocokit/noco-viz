<template>
  <div class="preview-screen" :style="{ background: screenBackground }">
    ...
    <!-- 缩放控制器（小屏幕时显示在右下角） -->
    <div v-if="windowSize.width < 1400" class="zoom-controls">
      <button class="zoom-btn" @click="handleRefresh" title="刷新">
        <svg viewBox="0 0 24 24" fill="currentColor">
          <path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/>
        </svg>
      </button>
...
const screenBackgroundImage = ref('')
...
const applyProjectData = (data) => {
  ...
  screenBackgroundImage.value = data.screenBackgroundImage || ''
  backgroundPosition.value = data.backgroundPosition || 'center center'
  backgroundWidth.value = data.backgroundWidth || ''
  backgroundHeight.value = data.backgroundHeight || ''
  cleanupAll()
  setupAll()
}
...
const canvasStyle = computed(() => {
  ...
  // 如果有背景图，添加背景图样式
  if (screenBackgroundImage.value) {
    style.backgroundImage = `url(${screenBackgroundImage.value})`
    style.backgroundRepeat = 'no-repeat'
    style.backgroundPosition = backgroundPosition.value || 'center center'

    // 处理背景尺寸
    if (backgroundWidth.value || backgroundHeight.value) {
      const width = backgroundWidth.value || 'auto'
      const height = backgroundHeight.value || 'auto'
